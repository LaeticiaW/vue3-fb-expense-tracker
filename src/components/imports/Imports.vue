...
<script setup lang="ts">
  ...
  const tableColumns: QTableColumn<Import>[] = [
    ...
    { name: 'actions', label: 'Actions', field: '_id', align: 'right', sortable: false },
  ]

  const rowText = computed(() => {
    return imports.length !== 1 ? 'rows' : 'row'
  })
  ...
  let importsQuery: ComputedRef<QueryResponse<Import[]>>
  try {
    importsQuery = useImports(filter)
    // queryLoading([importsQuery])
    queryLoading(importsQuery)
  } catch (err) {
    showNotify({ message: 'Error retrieving imports data' })
  }
  ...
  async function deleteImportedExpenses(importItem: Import) {
    showDialog({
      ...
      try {
        await ImportService.deleteImport(importItem._id)
